...
- Cada estudiante necesita un project (namespace) donde pueda crear
  recursos. Ese project se llama **igual que su usuario de login** (si
  el usuario es `user07`, el project es `user07`), y lo crea el
  instructor con `scripts/instructor-setup.sh <usuario>`, que además le
  otorga acceso `admin` sobre él. No se requiere cluster-admin para
  ningún task de estudiante.

## Estación de trabajo del estudiante

La forma recomendada (y la que asume esta guía) es el **Web Terminal**:
los estudiantes no instalan nada y trabajan enteramente desde el
navegador. Si el clúster no lo tiene disponible, `oc` local sigue siendo
una alternativa válida.

### Opción A (recomendada): Web Terminal, cero instalación local

Instala el **Web Terminal Operator** en el clúster (una sola vez, antes
del workshop):
...
`kubectl`, `helm`, etc.) preinstalados. Solo necesita un navegador, y
puede copiar/pegar los comandos de los labs directamente en esa
terminal.
...
### Opción B (alternativa): CLI local
...
y con sesión iniciada (idealmente la misma desde la que vas a
provisionar los projects):
...
Luego, para cada estudiante, crea su project (con el mismo nombre que su
usuario) y precarga los recursos de los Labs 1, 5 y 7:

```bash
./scripts/instructor-setup.sh <usuario>
```

Esto también le otorga al usuario acceso `admin` sobre su project. La
guía completa del instructor está en
[`docs/instructor-guide.md`](instructor-guide.md).
